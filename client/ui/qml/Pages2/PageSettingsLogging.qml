...
PageType {
    ...
    QtObject {
        ...
        readonly property string description: qsTr("RoninVPN logs")
        ...
        readonly property var exportLogsHandler: function() {
            var fileName = ""
            if (GC.isMobile()) {
                fileName = "RoninVPN.log"
            } else {
                fileName = SystemController.getFileName(qsTr("Save"),
                                                        qsTr("Logs files (*.log)"),
                                                        StandardPaths.standardLocations(StandardPaths.DocumentsLocation) + "/RoninVPN",
                                                        true,
                                                        ".log")
            }
            if (fileName !== "") {
    ...
    QtObject {
        ...
        readonly property string description: qsTr("RoninVPN-service logs")
        ...
        readonly property var exportLogsHandler: function() {
            ...
            fileName = SystemController.getFileName(qsTr("Save"),
                                                    qsTr("Logs files (*.log)"),
                                                    StandardPaths.standardLocations(StandardPaths.DocumentsLocation) + "/RoninVPN-service",
                                                    true,
                                                    ".log")
            if (fileName !== "") {
                PageController.showBusyIndicator(true)
